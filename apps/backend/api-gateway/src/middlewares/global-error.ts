...
export function globalErrorHandler(error: unknown, _req: Request, res: Response, _next: NextFunction) {
  ...
  if (error instanceof ApplicationError) {
    ...
    gatewayLogger.error(`$API Gateway - globalErrorHandler() method error: ${prettyObject(error)}`)
    return res.status(status).json({ message, error: errors })
  }

  // Unhandle Error
  gatewayLogger.error(`$API Gateway - globalErrorHandler() unexpected method error: ${prettyObject(error as {})}`)
