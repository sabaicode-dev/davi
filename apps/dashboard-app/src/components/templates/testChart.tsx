...
import Boolean from "../molecules/charts/BooleanChart";
...
const TestUI: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const numberColumn = apiResponse.find((item) => item.key === "6a9a9a34b74d4b61b15b9fe312786d65");
    const numberCounts = numberColumn?.table_column_metrics?.string_metrics?.counts || [];
    const limitedCounts = numberCounts.slice(0, 8);
    while (limitedCounts.length < 8) {
      limitedCounts.push({ key: `Extra ${limitedCounts.length + 1}`, value: 0 });
    }
    ...
    const categoryColumn = apiResponse.find((item) => item.key === "6bfb1786f7364b698ee202ba66135257");
    const categoryCounts = categoryColumn?.table_column_metrics?.string_metrics?.counts || [];
    const totalCategoryCount = categoryCounts.reduce((acc: number, item: { value: number }) => acc + item.value, 0);
    const processedCategories = categoryCounts.map((item: { key: string; value: number }) => ({
      category: item.key,
    ...
    const booleanColumn = apiResponse.find((item) => item.key === "boolean_column");
    const booleanCounts = booleanColumn?.table_column_metrics?.string_metrics?.counts || [];
    const booleanProcessed = {
      true: booleanCounts.find((item: { key: string }) => item.key === "True")?.value || 0,
      false: booleanCounts.find((item: { key: string }) => item.key === "False")?.value || 0,
    };
      ...
      <div className="flex-1 flex flex-row justify-center items-center gap-10 bg-gray-50">
        ...
        <div>
          <Number
            data={numberData}
            labels={numberLabels}
            onClick={() => handleBoxClick({ category: "Number Data", percentage: 50 })} // Example click
          />
        </div>
        ...
        <div>
          <Boolean
            data={booleanData}
            title="Boolean"
            onClick={() => handleBoxClick({ category: "Boolean Data", percentage: 60 })} // Example click
          />
        </div>
